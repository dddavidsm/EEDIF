...
const FORMATS: { key: Format; icon: string; name: string; desc: string }[] = [
  { key: 'xlsx', icon: 'XLS', name: 'Excel (XLSX)', desc: 'Hoja de calculo con resumen y lesiones por zona' },
  { key: 'pdf',  icon: 'PDF', name: 'Informe PDF', desc: 'Informe corporativo con fichas por lesion' },
  { key: 'csv',  icon: 'CSV', name: 'CSV', desc: 'Datos tabulados compatibles con cualquier programa' },
  { key: 'zip',  icon: 'ZIP', name: 'Backup ZIP', desc: 'Proyecto completo: datos JSON, XLSX y fotos' },
  { key: 'json', icon: 'JSN', name: 'JSON', desc: 'Datos estructurados para integracion con otros sistemas' },
]
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1.5">
          {FORMATS.map(f => (
            ...
            >
              <span className="inline-flex items-center justify-center w-11 h-9 rounded-[var(--radius)] border border-border bg-s1 text-[11px] font-mono font-bold text-t2">
                {f.icon}
              </span>
